  ...
  if (coding && seeded) {
    messages = [
      ...messages,
      {
        role: 'user',
        content:
          '[Системно] В проекте уже есть стартовый шаблон (src/App.jsx + src/styles.css отдельно). CSS только в .css, не внутри JSX. Не извиняйся вместо правок: write_file → check_build → только потом краткий ответ заказчику.',
      },
    ];
  }
